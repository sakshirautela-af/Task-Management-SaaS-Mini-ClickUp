...
import {sendSignupOtpEmail,sendForgetOtpEmail} from "../config/emailconfig.js"
...
export const initiateSignup = async (email) => {
  ...
  const a=await prisma.signupOtp.deleteMany({where:{
    email:email
}
  }) 
...
export const initiateForgetPassword = async (email) => {
  ...
  await prisma.forgetpassOtp.deleteMany({where:{
    email:email
}
  }) 
   const otp = generateOtp();
